...
class TestLoadGraphFromDistance:
    def test_estimates_duration_from_miles(self, tmp_path: Path):
        content = textwrap.dedent(
            """\
            Org Airport Code,Dest Airport Code,Aircraft,Distance
            KSEA,KSFO,B737,679
            KSFO,KLAX,A320,337
            """
        )
        ...
        G = load_graph(path)
        assert G.has_edge("KSEA", "KSFO")
        assert "duration" in G["KSEA"]["KSFO"]
        assert G["KSEA"]["KSFO"]["duration"] > 60
        assert G["KSEA"]["KSFO"]["distance"] == 679

    def test_duration_column_wins_over_distance(self, tmp_path: Path):
        content = textwrap.dedent(
            """\
            Org Airport Code,Dest Airport Code,Aircraft,DurationMinutes,Distance
            KSEA,KSFO,B737,110,679
            """
        )
        path = tmp_path / "both.csv"
        path.write_text(content, encoding="utf-8")
        G = load_graph(path)
        assert G["KSEA"]["KSFO"]["duration"] == 110

    def test_nm_column(self, tmp_path: Path):
        content = textwrap.dedent(
            """\
            Org Airport Code,Dest Airport Code,Aircraft,NM
            KDTW,KORD,A320,200
            """
        )
        ...
        assert G["KDTW"]["KORD"]["distance_unit"] == "nm"

    def test_fastest_uses_estimated_duration(self, tmp_path: Path):
        content = textwrap.dedent(
            """\
            Org Airport Code,Dest Airport Code,Aircraft,Miles
            KDTW,KDEN,B737,1200
            KDTW,KORD,B737,200
            KORD,KDEN,B737,900
            """
        ...
        path.write_text(content, encoding="utf-8")
        G = load_graph(path)
        best = find_shortest_by_time(G, "KDTW", "KDEN")
